...
public class Test implements ActionListener{
	...
	Test(){
		...
		frame.setTitle("測驗");		//sets title of frame 
        ...
        try {
            ...
            //2.建立statement類物件，用來執行SQL語句！！
            Statement statement = con.createStatement();
            String subject=ChooseTest.Subjects+"試卷";
            //要執行的SQL語句
            String SqlSelectTime="select `time` from `" +subject +"` where `group` = '" + ChooseTest.type + "';";
            //3.ResultSet類，用來存放獲取的結果集！！
            ...
            String SqlSelectExam="select `question` from `" + ChooseTest.Subjects +"` where `group` = '" + ChooseTest.type + "';";
            //3.ResultSet類，用來存放獲取的結果集！！
            ...
            String SqlExamNumber="select `examnumber` from `" + ChooseTest.Subjects +"試卷` where `group` = '" + ChooseTest.type + "';";
            //3.ResultSet類，用來存放獲取的結果集！！
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == handIn) {
	        ...
	        try {
	            ...
	            //要執行的SQL語句
	            int ti = 0;
	            String SqlSelectAnswer="select `answer` from `" + ChooseTest.Subjects +"` where `group` = '" + ChooseTest.type + "';";
	            //3.ResultSet類，用來存放獲取的結果集！！
	            ResultSet rsAnswer = statement.executeQuery(SqlSelectAnswer); 
	    	        ...
	    	        String SqlSelectStudentNumber="select `studentnumber` from `userdata` where `account` = '" + SignIn.userAccount + "';";
		            //3.ResultSet類，用來存放獲取的結果集！！
		            ResultSet rsNumber = statement.executeQuery(SqlSelectStudentNumber); 
		            if(rsNumber.next()) {
		            	studentNumber = rsNumber.getString("studentnumber");
		            }
	    	        String SqlUpdateGroup="update `"+ ChooseTest.Subjects +"群組` set `group` = '" + ChooseTest.type +"' where `studentnumber` = '" + studentNumber + "';";
		            //3.ResultSet類，用來存放獲取的結果集！！
		            statement.executeUpdate(SqlUpdateGroup); 
		            String SqlUpdateGrade="update `"+ ChooseTest.Subjects +"群組` set `grade` = '" + Grade +"' where `studentnumber` = '" + studentNumber + "';";
		            //3.ResultSet類，用來存放獲取的結果集！！
		            statement.executeUpdate(SqlUpdateGrade); 
	        } catch(ClassNotFoundException ex) {   
	            //資料庫驅動類異常處理
	            System.out.println("Sorry,can`t find the Driver!");   
	            ex.printStackTrace();   
