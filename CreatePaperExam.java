...
public class CreatePaperExam implements ActionListener{
	...
	CreatePaperExam(){
		...
		frame.setTitle("加入題目");		//sets title of frame 
		...
		if(e.getSource() == createExam) {
	        ...
	        if(check == 0) {
	        	try {
		            ...
		            Statement statement = con.createStatement();
		            //要執行的SQL語句
		            String createExam="update `"+ ExamPaperManage.Subject.charAt(0) + ExamPaperManage.Subject.charAt(1) +"` set `group` = '" + ChangePaperExam.Subject +"' where `question` = '" + inExam + "';";
		            //3.ResultSet類，用來存放獲取的結果集！！
		            ...
		            JOptionPane.showMessageDialog(null, "題目成功加入", "成功", JOptionPane.INFORMATION_MESSAGE); 
		            frame.dispose();
		        } catch(ClassNotFoundException ex) {   
		            //資料庫驅動類異常處理
		            System.out.println("Sorry,can`t find the Driver!");   
