...
public class ChooseTest implements ActionListener{
	...
	ChooseTest(){
		...
		frame.setTitle("挑選考試試卷");		//sets title of frame 
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == confirm) {
	        ...
	        try {
	            ...
	            String SqlSelectGrade="select `grade` from `" + ChooseTest.Subjects +"` where `studentnumber` = '" + studentNumber + "';";
	            //3.ResultSet類，用來存放獲取的結果集！！
	            ResultSet rsGrade = statement.executeQuery(SqlSelectGrade); 
	            	if(rsGrade.next()) {
		            	String b =rsGrade.getString("grade");
		            	check = Integer.parseInt(b);
		            }
	        } catch(ClassNotFoundException ex) {   
	            //資料庫驅動類異常處理
	            System.out.println("Sorry,can`t find the Driver!");   
			...
			if(Subjects.equals("") || Subjects.equals(" ") || Subjects.equals("null")) {
				...
				JOptionPane.showMessageDialog(null, "你已測驗過", "警告", JOptionPane.ERROR_MESSAGE);
			}
			else {
				frame.dispose();
				test = new Test();
