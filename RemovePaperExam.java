...
public class RemovePaperExam implements ActionListener{
	...
	RemovePaperExam(){
        ...
        backToChangePaperExam = new JButton("上一頁");
        backToChangePaperExam.setBounds(80,167, 80, 23);
        panel.add(backToChangePaperExam);
        backToChangePaperExam.addActionListener(this);
        
        removeExam = new JButton("移除題目");
        removeExam.setBounds(174,167, 80, 23);
        panel.add(removeExam);
        removeExam.addActionListener(this);
        
	}
	@Override
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == backToChangePaperExam) {
			frame.dispose();
		}
		if(e.getSource() == removeExam) {
			int check = 0;
	        ...
	        if(check == 0) {
	        	try {
		            ...
		            Statement statement = con.createStatement();
		            //要執行的SQL語句
		            String createExam="update `"+ ExamPaperManage.Subject.charAt(0) + ExamPaperManage.Subject.charAt(1) +"` set `group` = '無' where `question` = '" + inExam + "';";
		            //3.ResultSet類，用來存放獲取的結果集！！
		            statement.executeUpdate(createExam);
		            
		            int num = 0;
		            String SqlExamNumber="select `examnumber` from `" + ExamPaperManage.Subject +"` where `group` = '" + ChangePaperExam.Subject + "';";
		            //3.ResultSet類，用來存放獲取的結果集！！
		            ...
		            String updateexamnumber="update `"+ ExamPaperManage.Subject +"` set `examnumber` = '" + Num +"' where `group` = '" + ChangePaperExam.Subject + "';";
		            //3.ResultSet類，用來存放獲取的結果集！！
		            statement.executeUpdate(updateexamnumber);
		            
		            JOptionPane.showMessageDialog(null, "題目移除成功", "成功", JOptionPane.INFORMATION_MESSAGE); 
		            frame.dispose();
		        } catch(ClassNotFoundException ex) {   
		            //資料庫驅動類異常處理
		            System.out.println("Sorry,can`t find the Driver!");   
