...
public class RemovePaper implements ActionListener{
	...
	RemovePaper(){
		...
		frame.setTitle("刪除試卷");		//sets title of frame 
        ...
        inputFinishedNumber.setEditable(false);
        ...
        remove = new JButton("刪除");
        remove.setBounds(190,203, 80, 23);
		...
		if(e.getSource() == remove) {
	        ...
	        if(check == 0) {
	        	try {
	            	...
	            	String sqlAdd="delete from `"+ ExamPaperManage.Subject +"` where `group` = '" + inGroup + "';";
		            ...
		            //資料庫連線失敗異常處理
		            	JOptionPane.showMessageDialog(null, "刪除失敗", "失敗", JOptionPane.ERROR_MESSAGE);
		            	ex.printStackTrace();
		            }catch (Exception ex) {
		            // TODO: handle exception
		            ex.printStackTrace();
